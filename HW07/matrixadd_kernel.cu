...
{
   // ADD YOUR CODE HERE
   int xthread = blockIdx.x*blockDim.x + threadIdx.x;
   int ythread = blockIdx.y*blockDim.y + threadIdx.y;
   int index = xthread*gridDim.x*blockDim.x + ythread; 
   Pelems[index] = alpha*Melems[index] + beta*Nelems[index];
}
