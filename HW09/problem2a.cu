...
__global__ void ConvolutionKernel(Matrix M, Matrix N, Matrix P)
{
    ...
    */ 
    ...
    if(indexX<N.width && indexY<N.height){
        ...
        float sum = 0.0f;
        //if kernelSM contains the offset, we need to include offset in the index else we don't need to worry about it
        for(int i =currtop; i<currdown; i++){
            for(int j = currleft; j<currright;j++){
                sum += M.elements[i*M.width + j]*N.elements[(indexX+j-offset)+(indexY+i-offset)*N.width];
            }
        }
