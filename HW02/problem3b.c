...
int main(int argc, char*argv[])
{
	...
	FILE *fp;
	if((fp=fopen("problem3.dat", "r")) == NULL) {
	...
	*/
    ...
    //part b
    FILE *outfp;
	if((outfp=fopen("problem3.out", "w+")) == NULL) {
		fprintf(stderr,"Failed to write to the problem3.out\n");
		exit(1);
	}
    int *flipedMatrix = flipMatrix(imageMatrix, imageSize);
	printtoOutput(flipedMatrix, outfp, imageSize);
